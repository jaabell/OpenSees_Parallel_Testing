...
from gmshPostTools.gmshPostTools import write_nodes_scalar_data
import sys
...
filename = "./output_{}_{}/pressure_all.out".format(case, meshsize)

nodes = sp.loadtxt("./model_{}_{}/wetnodes.txt".format(case, meshsize))

print "Working on ", filename, " usestep = ", usestep, " nelem = ", nodes.size

fid_u = open( "./output_{}_{}/pressure.msh".format(case, meshsize), "w")
...
for step, line in enumerate(open(filename)):
    if step == usestep:
        ...
        write_nodes_scalar_data(fid_u, "Pressure", nodes, u, time=time, step=step, writeheader=writeheader)
